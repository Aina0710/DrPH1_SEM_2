...
```
# Interpretation
Using multivariate model with gender and hypertension as independent variables.

```{r}
mod4 <- vglm(cat_fbs2 ~ gender+ hpt, family = multinomial, data = clean_data)
summary(mod4)
```
The model show how gender and hypertension status influence the likelihood of being in the "Diabetes" or "Prediabetes" groups compared to the "Normal" group.
